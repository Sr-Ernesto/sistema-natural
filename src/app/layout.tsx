  ...
  return (
    <html lang="es">
      <head>
          ...
          {`
            (function(w,d,s,l,i){
              if (localStorage.getItem('osbrai_admin') === 'true') {
                console.log('OpenClaw: Admin detected, GTM disabled.');
                return;
              }
        ...
        <Script id="admin-detector" strategy="beforeInteractive">
          {`
            if (window.location.search.includes('osbrai_admin=1')) {
              localStorage.setItem('osbrai_admin', 'true');
              alert('Modo Invisible Activado: Tu tráfico ya no será medido.');
            }
          `}
        </Script>
      </head>
